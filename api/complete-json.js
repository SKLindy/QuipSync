...
// Force Node.js runtime by using (req, res) signature and res.json(...)
export default async function handler(req, res) {
  try {
    ...
    if (!['script','style'].includes(mode)) {
      return res.status(400).json({ error: 'Invalid mode (use "script" or "style")' });
    }
...
export default async function handler(req) {
  try {
    ...
    if (!['script','style'].includes(mode)) {
      return new Response(JSON.stringify({ error: 'Invalid mode (use "script" or "style")' }), { status: 400 });
    }
    ...
    return new Response(JSON.stringify({ error: 'LLM JSON endpoint failed', detail: String(e?.message || e) }), { status: 500 });
  }
}
